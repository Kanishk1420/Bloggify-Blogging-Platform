        ...
        <>
            ...
            <div className={`flex items-center justify-between px-4 md:px-[100px] lg:px-[200px] py-4 ${theme ? "bg-zinc-950 text-white" : "bg-white"}`}>
                        ...
                        <div className="relative w-full">
                            <div className={`absolute left-4 top-1/2 transform -translate-y-1/2 z-10`}>
                                <BsSearch 
                                    ...
                                    className={`cursor-pointer ${theme ? "text-blue-400" : "text-blue-600"}`} 
                                />
                            </div>
                            <input
                                type='text'
                                placeholder={path === '/finduser' ? 'Search users...' : 'Search...'}
                ...
                <div className='flex items-center space-x-2 md:space-x-4'>
                    {userInfo && (
                        <>
                            {/* Write button - now visible on ALL devices */}
                            <Link to='/write' className='flex items-center gap-1 sm:gap-2 cursor-pointer'>
                            ...
                            <div ref={menuRef} className="relative">
                                <button onClick={toggleNav} className="flex items-center justify-center">
                                    <img 
                                ...
                                <div
                                    className={`absolute right-0 mt-2 w-48 z-50 transform transition-all duration-300 origin-top-right
                                        ${menuState === 'closed' ? 'opacity-0 scale-95 pointer-events-none' :
                                        menuState === 'open' ? 'opacity-100 scale-100' :
                                        menuState === 'opening' ? 'opacity-100 scale-100' :
                                        'opacity-0 scale-95'}
                                    `}
                                >
                                    {(menuState === 'open' || menuState === 'opening' || menuState === 'closing') && <MobileMenu />}
                                </div>
                            </div>
                        </>
                    )}
